...
# --- 3. Boton Eye en fila (texto exacto con el bug de espacio) ---
...
assert old in c, "FAIL 3: boton editar no encontrado"
...
# --- 5. Opciones TV 3 valores — buscar con repr para ver encoding exacto ---
import re
m = re.search(r"\[\{v:1,l:'[^']+'\},\{v:0,l:'[^']+'\}\]\.map\(opt=>", c)
assert m, "FAIL 5: no se encontro el array de opciones TV"
old5 = m.group(0)
print("5. encontrado: " + old5)
new5 = "[{v:0,l:'Sin TV'},{v:30000,l:'MediTV Standard \u2014 $30.000'},{v:40000,l:'MediTV Premium \u2014 $40.000'}].map(opt=>"
...
new = "onChange={()=>setForm(p=>{const base=plans.find(x=>x.name===p.plan)?.value??(Number(p.plan_value)-Number(p.incluye_tv));return {...p,incluye_tv:opt.v,plan_value:base+opt.v}})}"
c = c.replace(old, new, 1)
...
if anchor not in c:
    anchor = "{/* == MODAL CLIENTE == */}"
assert anchor in c, "FAIL 8: anchor modal no encontrado — grep: {/* .* MODAL CLIENTE"
