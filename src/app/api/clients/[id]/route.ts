...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.execute({ sql: `UPDATE clients SET name=?,email=?,phone=?,cellphone=?,address=?,city=?,neighborhood=?,commune=?,consumption_date=?,payment_date=?,plan=?,plan_value=?,reference=?,status=?,notes=? WHERE id=?`, args: [body.name,body.email,body.phone,body.cellphone,body.address,body.city,body.neighborhood,body.commune,body.consumption_date,body.payment_date,body.plan,body.plan_value,body.reference,body.status,body.notes,id] })
