...
import { Wifi, Users, UserCheck, UserX, DollarSign, Plus, Trash2, Pencil, X, Tv,
         ...
         LayoutDashboard, Menu, ChevronLeft, Upload, Eye } from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { key:'plans',     label:'Planes',     icon: Wifi            },
    ...
    <div className="flex min-h-screen" style={{backgroundColor:BG, color:TEXT}}>
      ...
      <aside className={`fixed top-0 left-0 h-full z-50 flex flex-col lg:translate-x-0 transition-transform duration-300 ${sidebarOpen?'translate-x-0':'-translate-x-full'}`}
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
          <p className="px-3 mb-2 text-xs font-bold uppercase tracking-widest" style={{color:MUTED}}>Principal</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-[280px]">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between gap-3 px-4 md:px-6 h-16"
          ...
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-2 rounded-lg" style={{color:LIGHT,backgroundColor:CARD2,border:`1px solid ${BORDER}`}} onClick={()=>setSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-2">
            <div className="hidden lg:flex items-center gap-2 text-xs" style={{color:MUTED}}>
              <span>NIT 902060057-8</span><span style={{color:BORDER}}>·</span>
              <span>Mariana Lujan</span><span style={{color:BORDER}}>·</span>
      ...
      <main className="flex-1 p-4 sm:p-5 md:p-8 max-w-7xl w-full mx-auto space-y-5 md:space-y-6">
                    ...
                    {filteredClients.map(c=>(
                      <tr key={c.id} style={{borderBottom:`1px solid ${BORDER}`}} className="hover:bg-black/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          {c.incluye_tv>0?<p className="text-xs mt-0.5 font-semibold" style={{color:'#f59e0b'}}>+ MediTV</p>:null}
                        </td>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
              ...
              <F muted={MUTED} label="¿Incluye MediTV (TV satelital)?">
                <div className="flex items-center gap-3 mt-1">
                  {[{v:0,l:'Sin TV'},{v:30000,l:'MediTV Standard - $30.000'},{v:40000,l:'MediTV Premium - $40.000'}].map(opt=>(
